...
## TODO
## Check that lb <= ub
function safe_add!(name::String, ctx::Context)
...
ForallVar(T::Type{Float64}, lb::Float64, ub::Float64; name::String = genvar(), ctx::Context = global_context()) =
  (safe_add!(name, ctx); Ex{T}(opensmt_mk_forall_real_var(ctx.ctx, name, lb, ub), Set([name])))

ForallVar(T::Type{Float64}; name::String = genvar(), ctx::Context = global_context()) =
  (safe_add!(name, ctx); Ex{T}(opensmt_mk_forall_unbounded_real_var(ctx.ctx, name), Set([name])))

ForallVar(T::Type{Int64}, lb::Int64, ub::Int64; name::String = genvar(), ctx::Context = global_context()) =
  (safe_add!(name, ctx); Ex{T}(opensmt_mk_forall_int_var(ctx.ctx, name, lb, ub), Set([name])))

ForallVar(T::Type{Int64}; name::String = genvar(), ctx::Context = global_context()) =
